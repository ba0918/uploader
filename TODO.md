...
- [ ] `cuiConfirm()` の新ロジック（ターゲット毎表示）のテスト
  - 注: 主にUI出力なので統合テストでカバー済み（tests/integration/5_mirror_mode_test.ts）
...
- [ ] パフォーマンステストスクリプト作成
  - [ ] `tests/performance/manual-diff_test.ts` を作成
  - [ ] テストケース1: 小規模（100ファイル、各1KB）
  - [ ] テストケース2: 中規模（1,000ファイル、各10KB）
  - [ ] テストケース3: 大規模（8,000ファイル、各10KB）
  - [ ] テストケース4: 大容量（10ファイル、各100MB）
  - [ ] 実行時間とメモリ使用量を計測
- [ ] パフォーマンス結果の分析
  - [ ] rsync getDiff() との比較
  - [ ] 許容範囲の判定（例: 1000ファイルで5秒以内）
  - [ ] メモリ使用量の確認（例: 500MB以内）
- [ ] 必要に応じた最適化
  - [ ] ファイルサイズ制限の実装（例: 10MB以上はスキップ）
  - [ ] チャンク読み込みの検討
  - [ ] 進捗表示の追加

**所要時間**: 1.5日（テスト環境構築0.5日、計測・分析0.5日、最適化0.5日）
...
- [ ] `remote-diff.ts` のエラーハンドリング改善
  - [ ] 権限エラー（PermissionDenied）の区別
  - [ ] ファイル不在エラー（NotFound）の区別
  - [ ] ネットワークエラーの区別
  - [ ] より詳細なログ出力
- [ ] エラーログの改善
  - [ ] エラー種別の明示
  - [ ] エラー発生ファイルパスの記録
  - [ ] スタックトレースの保存（verbose時）
- [ ] テスト追加
  - [ ] 各エラー種別のハンドリング確認
  - [ ] エラー時の適切な動作検証
...
**問題**: rsyncの場合は`baseDirectory`を考慮した調整があるが、manual diffでは考慮されていない

**実装内容**:

- [ ] 動作差異の検証
  - [ ] rsyncとscp/sftp/localでmirrorモードの動作を比較
  - [ ] baseDirectory検出ロジックの統一が必要か判定
- [ ] 統一または差異の明確化
  - [ ] パターンA: manual diffにもbaseDirectory調整を追加
  - [ ] パターンB: 差異をドキュメントに明記
- [ ] 統合テストで検証
  - [ ] rsync vs scp/sftp/local の同一条件テスト
  - [ ] 結果の一致を確認
...
**問題**: `getManualDiffForTarget()` の制限事項やrsyncとの動作差異が文書化されていない
...
### 1. 統合テストの充実
- `tests/integration/5_mirror_mode_test.ts` で全プロトコル（rsync/sftp/scp/local）をテスト
- 501行の包括的なテストケース

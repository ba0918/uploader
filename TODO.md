...
## 進行中 (Phase 9: diff viewer 大規模ファイル対応)
...
### Phase 9.2: ツリー構造 + 遅延読み込み（推奨・優先度中） ✅ 完了
...
### Phase 9.3: rsync dry-run 差分検出（推奨・優先度中） ✅ 完了

**目的**: diff viewerに「実際に変更があるファイル」のみ表示

**背景**: 現状の`--diff=remote`は全ファイルを表示し、rsyncが内部で差分判定。
UIには8000ファイル表示されるが、実際の変更は数件という状況が発生。
...
# 出力例:
# >f+++++++++ new-file.txt      # 新規
# >f.st...... modified.txt      # 変更
# *deleting   deleted.txt       # 削除
```
...
1. **Phase 9.1** ✅ - 接続数制限（batchAsync）
2. **Phase 9.2** ✅ - ツリー構造 + 遅延読み込み
3. **Phase 9.2.5** ✅ - rsync一括アップロード（8000ファイル4秒）
4. **Phase 9.3** ✅ - rsync dry-run差分検出
5. **Phase 9.4** - 仮想スクロール（必要に応じて）

...
- [x] 1. `_global.ignore` を新構文に変更
- [x] 2. mirrorモード危険性の警告追加
- [x] 3. CLAUDE.md の更新
- [x] 4. README.md/SPEC.md の「後方互換」記述修正

### 🟡 中優先度（4項目）

- [x] 5. ターゲット固有の `ignore` 設定例追加
- [x] 6. 各設定項目の詳細説明充実
- [x] 7. プロトコル別制限事項の明記
- [x] 8. 実行例の追加
...
- `/home/mizumi/develop/inv/uploader.example.yaml` - 完全改訂（127行 → 271行、+144行）
...
- 基本的なプロファイル（development、staging）→ 高度な機能（production）→ 特殊ケース（legacy、local）
...
- ユーザーは `uploader.example.yaml` をコピーするだけで即座に動作する設定を得られる
- 各設定項目の意味が明確になり、カスタマイズが容易に
- よくある間違いを事前に防止し、サポート負荷を軽減
- プロトコル選択の判断材料を提供し、最適な構成が可能に

このレポートは `/home/mizumi/develop/inv/docs/review/uploader-example-yaml-improvement-report.md` に保存されているのだ。

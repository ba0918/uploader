...
実行時に指定するプロファイル（`uploader <profile>`）の設定なのだ。
...
未追跡ファイル（git add されていないファイル）を含めるかなのだ。
...
アップロード対象のファイル・ディレクトリ（globパターン対応）なのだ。

**重要**: 末尾スラッシュの有無で動作が変わるのだ。
...
全ターゲット共通の設定なのだ。個別ターゲットで上書き可能なのだ。
...
**重要**: `dest` は各ターゲットで個別に指定する必要があるのだ（defaults
に含められないのだ）。
...
SSH秘密鍵のパスなのだ。`~` はホームディレクトリに展開されるのだ。
...
**警告**:
mirrorモードは、ローカルに存在しないファイルをリモートから**自動削除**するのだ。必ず
`--dry-run` で確認してほしいのだ。
...
ファイル権限を維持するか（rsync/localのみ有効）なのだ。
...
タイムスタンプを維持するか（rsync/localのみ有効）なのだ。
...
古いSSHアルゴリズムを有効化（レガシーサーバー向け）なのだ。
...
リモート側で実行するrsyncコマンドなのだ。sudo権限が必要な場合に使用するのだ。

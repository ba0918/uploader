...
uploaderツールの実践的な使用例とベストプラクティスをまとめました。
自分のユースケースに合った設定を見つけて、効率的なデプロイを実現してください。
...
小規模なWebサイト（HTML/CSS/JavaScript）をレンタルサーバーにデプロイする最もシンプルなケースです。
Git管理されているプロジェクトで、mainブランチとの差分のみを安全にアップロードします。
...
- **sync_mode: update** により、既存ファイルは削除されないため安全です。
- **--dry-run** で事前確認してから実行するのがベストプラクティスです。
- **--diff**
  オプションでブラウザ上で視覚的に差分を確認できるので、ミスを防げます。
- **include_untracked: false** により、git
  addしていないファイルは誤ってアップロードされません。
...
開発環境、ステージング環境、本番環境の3段階でWebアプリケーションをデプロイするケースです。
各環境で異なる除外パターンを適用し、段階的に安全にリリースします。
...
- **環境ごとにignoreパターンを変える**ことで、適切なファイルのみをデプロイできます。
- **本番環境は--diffで必ず確認**してから実行するのがベストプラクティスです。
- **環境変数**を使うことで、認証情報をYAMLに直接書かずに済みます。
- **base**を環境ごとに変える（develop/main）ことで、適切な差分を検出できます。
...
ロードバランサー配下の複数のWebサーバに同じファイルをデプロイするケースです。
defaultsを使って共通設定を定義し、冗長性を減らします。
...
- **defaults**を使うことで、設定の重複を大幅に削減できます。
- **rsyncプロトコル**は複数サーバへの同時デプロイで特に高速です。
- **--compress**
  オプションで帯域を節約できるが、CPUを使うので環境に応じて調整します。
- **個別にtimeoutを上書き**できるので、遅いサーバだけ調整できます。
- 1つのサーバが失敗しても他のサーバは続行されるので安全です。
...
数千ファイル以上の大規模プロジェクトを高速にデプロイするケースです。
rsyncの差分転送とsudo権限を使って、効率的かつ安全にデプロイします。
...
- **rsyncプロトコル**は大規模プロジェクトで圧倒的に高速です（SFTP/SCPの10倍以上速い場合もあります）。
- **rsync_path: "sudo rsync"** により、リモート側で権限問題を回避できます。
- **--chmod/--chown**
  でパーミッション・所有者を一括設定できます（手動chmodが不要になります）。
- **preserve_permissions**
  により、ローカルのパーミッションをそのまま維持できます。
- **--compress**
  は低速回線で有効ですが、高速LAN内では無効化した方が速い場合もあります。
...
本番サーバーのファイルをローカルにバックアップし、必要に応じてリストアするケースです。
localプロトコルを使って、リモート→ローカルのコピーも実現できます。
...
- **localプロトコル**を使えば、リモート転送なしでローカルコピーができます。
- **sync_mode: mirror**
  により、バックアップ先とソースを完全一致させることができます。
- **preserve_permissions/timestamps** により、元のファイル属性を保持できます。
- **日付付きディレクトリ名**で履歴管理できます（`$(date
  +%Y%m%d)`はシェル展開です）。
- リストア前は**必ず--dry-runと--diff**で確認してください。
...
CentOS 6、Ubuntu 14.04などの古いSSHサーバーにデプロイするケースです。
legacy_modeを有効化して、古いSSHアルゴリズムでも接続できるようにします。
...
- **legacy_mode: true**により、古いSSHサーバーでも接続できるようになります。
- **scpプロトコル**は古い環境でも安定して動作します。
- **パスワード認証**は環境変数で管理し、YAMLに直接書かないでください。
- **timeout/retryを多め**に設定すると、接続の不安定さに対応できます。

### セキュリティ上の注意

- **legacy_modeのアルゴリズムは古く、セキュリティリスクがあります**。
- 可能な限り**サーバーのアップデートを推奨**します。
- **本番環境では使用を避け**、一時的な移行期間のみ使用します。
- **VPN経由の接続を検討**してセキュリティを高めます。

### SSH設定で対応する場合

uploader設定だけでなく、SSH設定でも対応できます。
...
この場合、uploader設定で`legacy_mode: false`でも接続できます。
...
本番環境でmirrorモードを実行する前に、ローカル環境で安全に動作確認するケースです。
localプロトコルを使って、リスクなしにmirrorモードの挙動を確認できます。
...
- **localプロトコル**でリスクなしにテストできます。
- **mirrorモード**は削除を伴うので、**必ず事前にローカルテスト**してください。
- **--dry-runと--diff**の両方で確認することで、削除ミスを防げます。
- **ログファイルに保存**することで、削除履歴を記録できます。
- テストで期待通りなら、同じ設定を本番環境に適用できます。
...
Git管理されているプロジェクトで、ブランチ間の差分のみを効率的にデプロイするケースです。
CLIオプションで柔軟にブランチを切り替えて、必要な差分だけをアップロードします。
...
- **gitモード**により、変更されたファイルのみを自動検出してアップロードできます。
- **CLIオプション（--base/--target）**により、設定ファイルを変えずに柔軟にブランチを指定できます。
- **include_untracked**を使えば、git addしていない新規ファイルも含められます。
- **rsyncプロトコル**と組み合わせることで、高速な差分デプロイが可能です。
- **タグ間の差分**を指定することで、リリース履歴を明確に管理できます。
...
各ユースケースで紹介した設定例は、実際に動作する構成です。
自分のプロジェクトに合わせてカスタマイズして、効率的なデプロイを実現してください。
